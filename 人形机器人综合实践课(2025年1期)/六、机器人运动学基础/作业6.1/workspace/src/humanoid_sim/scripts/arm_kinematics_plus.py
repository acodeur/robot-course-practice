...
用法:
  arm_kinematics_plus.py fk <q1> <q2> <l/r>        正运动学
  arm_kinematics_plus.py ik <s> <d> <l/r>          逆运动学 (2R 平面坐标)
  arm_kinematics_plus.py ik --torso <y> <z> <l/r>  逆运动学 (torso 坐标系)
  arm_kinematics_plus.py reach <s> <d>             可达性检查
  arm_kinematics_plus.py multi <s> <d>             多解对比 (肘弯曲 vs 肘反向)

示例:
  arm_kinematics_plus.py fk 0.8 -0.9
  arm_kinematics_plus.py fk 0.8 -0.9 r
  arm_kinematics_plus.py ik 0.10 0.30
  arm_kinematics_plus.py ik 0.10 0.30 r
  arm_kinematics_plus.py ik --torso 0.15 0.10
  arm_kinematics_plus.py ik --torso 0.15 0.10 r
  arm_kinematics_plus.py reach 0.10 0.50
  arm_kinematics_plus.py multi 0.10 0.30
"""
import math
import sys
...
def to_torso(s, d, lr='l'):
    """(s, d) → torso 坐标系 (x, y, z)"""
    return (L_SHOULDER_IN_TORSO[0], s, L_SHOULDER_IN_TORSO[2] - d) if lr == 'l' else (R_SHOULDER_IN_TORSO[0], s, R_SHOULDER_IN_TORSO[2] - d)


def rostopic_cmd(q1, q2, lr='l'):
    if lr == 'l':
        ...
        return (f'rostopic pub /humanoid/position_controller/command '
            f'std_msgs/Float64MultiArray '
            f'"data: [0.0, {q1:.4f}, 0.0, {q2:.4f}, 0.0, 0.0, 0.0, 0.0]" -1')
...
def cmd_fk(args):
    ...
    lr = args[2] if len(args) > 2 else 'l'
    ...
    print()
    print(f"{'左' if lr == 'l' else '右'}手端 (torso 系): x={x:.4f}, y={y:.4f}, z={z:.4f}")
    print(f"  ↑ 应与 tf_echo /torso {'/left_hand' if lr == 'l' else '/right_hand'} 的 Translation 一致")
    print()
    print(f"2R 平面坐标: s={s:.4f} (侧向), d={d:.4f} (下垂)")
    print("=" * 55)
    print()
    print(">>> 对应的关节命令:")
    print(rostopic_cmd(q1, q2, lr))
...
def cmd_ik(args):
    ...
    if len(args) >= 3 and args[0] == "--torso":
        ...
        if len(args) >= 4:
            lr = args[3]
        d_target = (L_SHOULDER_IN_TORSO[2] - z_t) if lr == 'l' else (R_SHOULDER_IN_TORSO[2] - z_t)
    ...
    s_fk, d_fk = fk_2r(q1, q2, lr)
    err = math.sqrt((s_fk - s_target) ** 2 + (d_fk - d_target) ** 2)
    _, y_fk, z_fk = to_torso(s_fk, d_fk, lr)

    q1_ok = Q1_LIMITS[0] <= q1 <= Q1_LIMITS[1]
    q2_ok = Q2_LIMITS[0] <= q2 <= Q2_LIMITS[1]

    print("=" * 55)
    print(f"{'左手' if lr == 'l' else '右手'}目标: s = {s_target:.4f} m, d = {d_target:.4f} m")
    ...
    print(f"末端 torso 坐标: x={L_SHOULDER_IN_TORSO[0] if lr == 'l' else R_SHOULDER_IN_TORSO[0]}, y={y_fk:.4f}, z={z_fk:.4f}")
    print()
    if q1_ok and q2_ok:
